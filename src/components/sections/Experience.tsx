      ...
      <ol className="relative border-l border-surge/40 pl-6">
        {experience.map((item) => (
          <li key={item.id} className="mb-12">
            ...
            {/* Bullet points */}
            <ul className="mt-4 space-y-2 text-skin-fg/80">
              {item.bullets
            ...
            {item.bullets.length > 2 && (
              <button
                ...
                className="mt-2 text-sm font-medium text-surge hover:text-indigo hover:underline"
              >
            ...
            {item.quote && openId === item.id && (
              <blockquote className="mt-4 border-l-4 border-surge/60 pl-4 italic text-gray-500 dark:text-gray-400">
